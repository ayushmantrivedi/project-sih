...
if (-not $dockerInstalled) {
    ...
    Write-Host "`nPlease install Docker Desktop for Windows:" -ForegroundColor Yellow
    ...
    Write-Host "`nAfter installing Docker, run this script again." -ForegroundColor Yellow
...
if ($missingFiles.Count -gt 0) {
    Write-Host "`nMissing required files. Please ensure all files are present." -ForegroundColor Red
    exit 1
}

# Build and run the application
Write-Host "`nBuilding and starting HealthBot..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Health check failed or service not ready yet" -ForegroundColor Yellow
    Write-Host "You can check the logs with: docker-compose logs healthbot" -ForegroundColor Cyan
}

# Display success message
Write-Host "`nHealthBot AI Chatbot is now running!" -ForegroundColor Green
Write-Host "====================================" -ForegroundColor Green
...
Write-Host "`nYour HealthBot AI Chatbot is ready to use!" -ForegroundColor Green
